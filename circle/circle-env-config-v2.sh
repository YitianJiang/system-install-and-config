...
cp $SYSTEM_INSTALL_AND_CONFIG/circle/mysql/my.cnf /mydata/mysql/data/conf
...
#配置mysql
docker cp $SYSTEM_INSTALL_AND_CONFIG/circle/mysql/circle.sql mysql:/
ConfigMysql(){
sleep 10
...
# 使用logstash-input-jdbc配置es全量同步mysql 
docker cp $SYSTEM_INSTALL_AND_CONFIG/circle/logstash/mysql-connector-java-8.0.15.jar logstash:/
docker cp $SYSTEM_INSTALL_AND_CONFIG/circle/logstash/jdbc.sql logstash:/
docker restart logstash

#使用canal配置es增量同步mysql
bash $SYSTEM_INSTALL_AND_CONFIG/circle/canal/deploy.sh
